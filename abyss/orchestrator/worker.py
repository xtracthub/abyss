...
class Worker:
    ...
    def validate_dict_params(worker_params: dict) -> None:
        ...
        try:
            ...
            assert len(worker_params) == len(REQUIRED_WORKER_PARAMETERS), \
                f"Expected {len(REQUIRED_WORKER_PARAMETERS)} parameters got {len(worker_params)}"
        except AssertionError as e:
            raise ValueError(str(e))
        except KeyError:
            raise ValueError(f"Worker parameter {parameter_name} not found")
