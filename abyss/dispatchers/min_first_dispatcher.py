...
class MinFirstDispatcher(Dispatcher):
    def __init__(self, workers: List[Worker]):
        """Dispatches worker_batches into queues min first.
    ...
    def dispatch_batch(self, worker_batches: dict) -> None:
        """Places worker_batches into queues for each worker in min
        first ordering.
    ...
    def _dispatch(self) -> None:
        """Places items from worker batch into worker queue in min first
        ordering.
        ...
        for worker_id, worker_batch in self.worker_batches.items():
            ...
            worker_batch.sort(reverse=False,
